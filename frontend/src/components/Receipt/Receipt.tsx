  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setHistoryMovements(data.filter((m: any) => m.movement_type === "Bevételezés"));
      }
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ fontSize: '24px', margin: 0 }}>Bevételezés</h1>
        <button 
          onClick={() => {
      ...
      {showHistoryModal && (
        ...
        >
          <div className="glass-panel" style={{ padding: '24px', width: '900px', maxWidth: '95vw', display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: '12px' }}>
              <h3 style={{ margin: 0, color: '#38bdf8' }}>Korábbi bevételezések</h3>
            ...
            {historyLoading ? (
              ...
              <div style={{ padding: '20px', textAlign: 'center', color: '#94a3b8' }}>Nincs korábbi bevételezés.</div>
            ) : (
                  ...
                  <tbody>
                    {historyMovements.map((m, idx) => (
                      <tr key={idx}>
                        ...
                        <td style={{ fontWeight: 'bold', color: '#4ade80' }}>+{m.quantity_delta} db</td>
                        <td>{m.price_net ? `${new Intl.NumberFormat('hu-HU', { style: 'currency', currency: 'HUF', maximumFractionDigits: 0 }).format(m.price_net)}` : '-'}</td>
