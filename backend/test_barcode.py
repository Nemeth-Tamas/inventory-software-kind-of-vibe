...
async def test_barcode_endpoint_security_and_preview():
        ...
        await session.execute(
            delete(AuditLog).where(
                AuditLog.username.in_(
                    ["barcode_admin", "barcode_unauth", "barcode_warehouse"]
                )
            )
        )
        await session.execute(
    ...
    try:
        async with httpx.AsyncClient(
            ...
            login_unauth = await client.post(
                "/auth/login",
                data={"username": "barcode_unauth", "password": "unauth123"},
            )
            assert login_unauth.status_code == 200
            unauth_token = login_unauth.json()["access_token"]

            login_wh = await client.post(
                "/auth/login",
                data={"username": "barcode_warehouse", "password": "wh123"},
            )
