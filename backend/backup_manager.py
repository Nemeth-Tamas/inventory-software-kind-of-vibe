...
def sanitize_and_validate_backup_filename(filename: str) -> str:
    ...
    if "/" in filename or "\\" in filename or ".." in filename:
        raise ValueError(
            "Invalid backup filename: path traversal or directory components are not allowed."
        )
    ...
    if not pattern.match(filename):
        raise ValueError(
            "Invalid backup filename: name does not match expected patterns (e.g. inventory_YYYY-MM-DD_HHMMSS.dump)."
        )
            ...
            with open(HISTORY_FILE, "r") as f:
                ...
                for h in reversed(hist):
                    if (
                        h["result"] == "success"
                        and h["filename"]
                        and h["filename"].startswith("inventory_")
                    ):
                        last_success = h
                        break
    except Exception:
...
async def run_restore_temp(filename, latest=False):
    ...
    print(
        f"Restoring backup {filename} into temporary verification database: {temp_db_name}..."
    )
    ...
    try:
        ...
        try:
            ...
            schema_version = await v_conn.fetchval(
...
async def run_restore_live(filename, confirm):
    ...
    if not confirm:
        print(
            "Error: Live restore requires explicit confirmation. Add the --confirm flag."
        )
        sys.exit(1)
    ...
    if not safety_ok:
        print(
            f"Error: Safety backup failed ({safety_file}). Live restore aborted for safety."
        )
        sys.exit(1)
    ...
    try:
        ...
        await conn.execute(
            f"REVOKE CONNECT ON DATABASE {target_db} FROM {DB_CREDS['user']}"
        )
        await conn.execute(
    ...
    try:
        await conn.execute(
            f"GRANT CONNECT ON DATABASE {target_db} TO {DB_CREDS['user']}"
        )
        await conn.execute(f"GRANT CONNECT ON DATABASE {target_db} TO public")
        await conn.execute(
            f"GRANT ALL PRIVILEGES ON DATABASE {target_db} TO {DB_CREDS['user']}"
        )
    finally:
        await conn.close()
        ...
        try:
            ...
            if now_local.hour >= 2:
                if not check_daily_backup_exists_for_date(today_str):
                    print(
                        f"Daily backup not found for date {today_str}. Running scheduled backup..."
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Inventory System Backup and Restore Utility"
    )
    ...
    verify_p = subparsers.add_parser(
        "verify-backup", help="Verify a backup file structure"
    )
    verify_p.add_argument(
        "filename", nargs="?", default=None, help="Backup filename to verify"
    )
    verify_p.add_argument(
        "--latest", action="store_true", help="Verify the latest successful backup"
    )

    rest_temp_p = subparsers.add_parser(
        "restore-temp", help="Restore backup into temporary database for validation"
    )
    rest_temp_p.add_argument(
        "filename", nargs="?", default=None, help="Backup filename to verify restore on"
    )
    rest_temp_p.add_argument(
        "--latest", action="store_true", help="Restore the latest successful backup"
    )

    rest_live_p = subparsers.add_parser(
        "restore-live", help="Restore backup to live database"
    )
    rest_live_p.add_argument("filename", help="Backup filename to restore")
    rest_live_p.add_argument(
        "--confirm",
        action="store_true",
        help="Explicit confirmation for live overwrite",
    )
    ...
    if args.command == "run-backup":
        ...
        if not args.filename and not args.latest:
            parser.error(
                "verify-backup requires either a filename or the --latest flag."
            )
        asyncio.run(verify_command(args.filename, args.latest))
    elif args.command == "restore-temp":
        if not args.filename and not args.latest:
            parser.error(
                "restore-temp requires either a filename or the --latest flag."
            )
