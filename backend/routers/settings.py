...
async def get_backup_status(current_user: User = Depends(require_role([UserRole.ADMIN]))):
    # Read status file
    status_info = {}
            ...
            for filename in os.listdir(BACKUP_DIR):
                if pattern.match(filename):
                    ...
                    files_list.append({
                        ...
                        "created_at": datetime.fromtimestamp(os.path.getctime(fp), TZ).isoformat()
                    })
        except Exception:
            pass
    status_info["backup_files"] = sorted(files_list, key=lambda x: x["filename"], reverse=True)
...
async def run_backup_now(current_user: User = Depends(require_role([UserRole.ADMIN]))):
    ...
    if success:
        ...
        raise HTTPException(status_code=500, detail=f"Mentés sikertelen: {result_detail}")

@router.post("/backup/verify/{filename}")
async def verify_backup_now(filename: str, current_user: User = Depends(require_role([UserRole.ADMIN]))):
    # Prevent path traversal
    if not re.match(r"^(inventory|safety_inventory)_\d{4}-\d{2}-\d{2}_\d{6}\.dump$", filename):
    ...
    try:
        ...
        return {"status": "success", "message": f"A(z) {filename} mentés sikeresen ellenőrizve."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Ellenőrzés hiba: {str(e)}")

@router.get("/backup/download/{filename}")
async def download_backup(filename: str, current_user: User = Depends(require_role([UserRole.ADMIN]))):
    # Prevent path traversal
    if not re.match(r"^(inventory|safety_inventory)_\d{4}-\d{2}-\d{2}_\d{6}\.dump$", filename):
        raise HTTPException(status_code=400, detail="Invalid filename")
