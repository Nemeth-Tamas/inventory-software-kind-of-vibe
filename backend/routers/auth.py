...
def validate_password_strength(password: str):
    ...
    common_passwords = {"password", "12345678", "admin123", "qwertyui", "jelszo123", "password123"}
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    ...
    if not user or not verify_password(form_data.password, user.hashed_password):
        ...
        await log_audit(db, None, form_data.username, "Sikertelen bejelentkezési kísérlet (hibás jelszó)")
        await db.commit()
        raise HTTPException(
...
async def reset_user_password(
    ...
    await log_audit(db, current_user.id, current_user.username, f"Adminisztrátori jelszó-visszaállítás a következő felhasználónál: {user.username}")
    await db.commit()
    return {"status": "success", "message": f"A(z) {user.username} felhasználó jelszava sikeresen visszaállítva. Kötelező csere beállítva."}
