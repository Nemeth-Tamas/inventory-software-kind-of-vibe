...
async def create_product(prod: ProductCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER, UserRole.WAREHOUSE]))):
    ...
    try:
        if not prod.barcode:
            ...
            result = await db.execute(select(Product).where(Product.barcode == prod.barcode))
            if result.scalar_one_or_none():
                raise HTTPException(status_code=400, detail="Ez a vonalkód már létezik!")
            barcode = prod.barcode
            
        # Check SKU uniqueness (only if SKU is provided)
        if prod.sku and prod.sku.strip():
            sku_result = await db.execute(select(Product).where(Product.sku == prod.sku.strip()))
            if sku_result.scalar_one_or_none():
                raise HTTPException(status_code=400, detail="Ez a cikkszám (SKU) már létezik!")

        # Validate and normalize prices
        purchase_net = prod.purchase_price_net
        purchase_gross = normalize_gross(purchase_net, prod.purchase_price_gross, prod.vat_rate)
        ...
        await log_audit(db, current_user.id, current_user.username, f"Termék létrehozva: {new_prod.name} (Vonalkód: {new_prod.barcode})")
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"A termék mentése közben adatbázishiba történt: {str(e)}")
        
    # Trigger Server-Sent Event for real-time dashboard / list update
    await event_bus.publish("inventory_events", "product_created", {"id": loaded_prod.id, "name": loaded_prod.name, "barcode": loaded_prod.barcode})
...
async def update_product(product_id: str, prod: ProductCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER, UserRole.WAREHOUSE]))):
    try:
        result = await db.execute(select(Product).where(Product.id == product_id))
        ...
        new_barcode = prod.barcode.strip() if (prod.barcode and prod.barcode.strip()) else db_prod.barcode
        if new_barcode and new_barcode != db_prod.barcode:
            barcode_result = await db.execute(select(Product).where(Product.barcode == new_barcode))
            if barcode_result.scalar_one_or_none():
                raise HTTPException(status_code=400, detail="Ez a belső vonalkód már létezik!")
        ...
        await log_audit(db, current_user.id, current_user.username, f"Termék módosítva: {db_prod.name} (id: {product_id})")
        await db.commit()
    except Exception as e:
        await db.rollback()
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"A termék módosítása közben adatbázishiba történt: {str(e)}")
        
    await event_bus.publish("inventory_events", "product_updated", {"id": db_prod.id, "name": db_prod.name})
    return db_prod

@router.delete("/delete-zero-stock")
async def delete_zero_stock_products(db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER]))):
    from sqlalchemy import delete
    from models_inventory import StocktakeItem, StocktakeUnknownBarcode, InventoryMovement
    
    try:
        # Get IDs of products that have current_stock == 0
        zero_stock_result = await db.execute(select(Product.id).where(Product.current_stock == 0))
        product_ids = [r[0] for r in zero_stock_result.all()]
        
        if not product_ids:
            return {"status": "success", "message": "Nincsenek 0 készletes termékek.", "deleted_count": 0}
            
        # Delete related records to avoid FK violations
        await db.execute(delete(InventoryMovement).where(InventoryMovement.product_id.in_(product_ids)))
        await db.execute(delete(StocktakeItem).where(StocktakeItem.product_id.in_(product_ids)))
        await db.execute(delete(StocktakeUnknownBarcode).where(StocktakeUnknownBarcode.resolved_product_id.in_(product_ids)))
        
        # Finally, delete the products
        await db.execute(delete(Product).where(Product.id.in_(product_ids)))
        
        await log_audit(db, current_user.id, current_user.username, f"Összes 0 készletes termék törölve ({len(product_ids)} db)")
        await db.commit()
        
        # Broadcast event
        await event_bus.publish("inventory_events", "products_bulk_deleted", {"deleted_count": len(product_ids)})
        
        return {"status": "success", "message": f"Sikeresen törölve {len(product_ids)} darab 0 készletes termék.", "deleted_count": len(product_ids)}
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"Hiba a termékek törlése során: {str(e)}")

@router.delete("/{product_id}")
async def delete_product(product_id: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER]))):
    from sqlalchemy import delete
    from models_inventory import StocktakeItem, StocktakeUnknownBarcode, InventoryMovement
    ...
    try:
        ...
        await db.execute(delete(InventoryMovement).where(InventoryMovement.product_id == product_id))
        await db.execute(delete(StocktakeItem).where(StocktakeItem.product_id == product_id))
        await db.execute(delete(StocktakeUnknownBarcode).where(StocktakeUnknownBarcode.resolved_product_id == product_id))
        
        # Delete product
        await db.execute(delete(Product).where(Product.id == product_id))
        
        await log_audit(db, current_user.id, current_user.username, f"Termék törölve: {db_prod.name} (id: {product_id}, vonalkód: {db_prod.barcode})")
        await db.commit()
        
        await event_bus.publish("inventory_events", "product_deleted", {"id": product_id, "name": db_prod.name})
