...
async def execute_queue_item(
    db: AsyncSession, item: BillingoQueueItem, api_key: str
) -> bool:
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code in (200, 201):
                ...
                item.error_message = (
                    f"API válasz: {response.status_code} - {response.text}"
                )
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                "https://api.billingo.hu/v3/partners", headers=headers, timeout=5.0
            )
...
async def queue_product_sync(
    ...
    payload = {
        ...
        "net_unit_price": float(product.sale_price_net)
        if product.sale_price_net
        else 0.0,
    ...
    q_result = await db.execute(
        select(BillingoQueueItem).where(
            (BillingoQueueItem.product_id == product_id)
            & (BillingoQueueItem.status == BillingoQueueStatus.PENDING)
        )
    )
    existing_item = q_result.scalar_one_or_none()

    action = (
        BillingoQueueAction.UPDATE
        if product.billingo_product_id
        else BillingoQueueAction.CREATE
    )
    ...
    await log_audit(
        ...
        f"Billingo szinkronizációs kérés sorba állítva: {product.name}",
    )
    return {
        "status": "success",
        "message": "A szinkronizációs kérés sikeresen sorba állítva felülvizsgálatra.",
    }
...
async def process_queue_item(
    ...
    if not api_key:
        if settings.ALLOW_MOCK_BILLINGO:
            ...
            await log_audit(
                ...
                current_user.id,
                current_user.username,
                f"Billingo szimulált szinkron feldolgozva: {item.product.name}",
            )
            return {"status": "success", "message": "Szimulációs szinkronizáció kész."}
        else:
            raise HTTPException(
                status_code=400, detail="A Billingo API kulcs hiányzik."
            )
    ...
    if success:
        await log_audit(
            ...
            current_user.id,
            current_user.username,
            f"Billingo szinkronizáció végrehajtva: {item.product.name}",
        )
        return {
            "status": "success",
            "message": "Sikeresen szinkronizálva a Billingo-val.",
        }
    else:
        raise HTTPException(
            status_code=400, detail=f"Sikertelen szinkronizáció: {item.error_message}"
        )
...
async def process_all_queue_items(
    ...
    if not items:
        return {
            "status": "success",
            "message": "Nincs feldolgozandó elem a sorban.",
            "processed_count": 0,
        }
    ...
    await log_audit(
        ...
        current_user.id,
        current_user.username,
        f"Billingo tömeges szinkronizáció lefutott: {processed_count} sikeres, {failed_count} sikertelen",
    )
    return {
        "status": "success",
...
async def delete_queue_item(
    ...
    item.product.billingo_sync_state = (
        "Nincs szinkronizálva"
        if not item.product.billingo_product_id
        else "Szinkronban"
    )
    ...
    return {
        "status": "success",
        "message": "A szinkronizációs kérés eltávolítva a sorból.",
    }


@router.post("/import")
async def import_products_from_billingo(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER])),
):
    api_key = await get_billingo_api_key(db)
    if not api_key:
        raise HTTPException(
            status_code=400,
            detail="A Billingo API kulcs hiányzik. Kérjük, állítsa be a beállítások menüben!",
        )
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            while True:
                response = await client.get(
                    f"https://api.billingo.hu/v3/products?page={page}&per_page=100",
                    headers=headers,
                    timeout=10.0,
                )
                if response.status_code != 200:
                    raise HTTPException(
                        status_code=400, detail=f"Billingo API hiba: {response.text}"
                    )
                ...
                for bp in billingo_products:
                    ...
                    p_res = await db.execute(
                        select(Product).where(
                            (Product.billingo_product_id == bp_id)
                            | (Product.sku == bp_sku)
                        )
                    )
                    ...
                    if existing_product:
                        ...
                        bp_ean = bp.get("ean")
                        cleaned_ean = (
                            bp_ean.strip() if (bp_ean and bp_ean.strip()) else None
                        )

                        # We only use EAN directly as the barcode if it fits our 6-character database limit
                        if (
                            cleaned_ean
                            and len(cleaned_ean) == 6
                            and cleaned_ean not in existing_barcodes
                        ):
                            barcode = cleaned_ean
                        else:
                            from barcode_utils import generate_next_barcode

                            barcode = await generate_next_barcode(
                                db, exclude_barcodes=existing_barcodes
                            )
        ...
        await log_audit(
            ...
            current_user.id,
            current_user.username,
            f"Billingo termék importálás lefutott: {imported_count} új, {updated_count} frissített termék",
        )

    except Exception as e:
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Hiba a termékimportálás során: {str(e)}"
        )
