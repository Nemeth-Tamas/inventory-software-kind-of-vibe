...
async def export_products(db: AsyncSession = Depends(get_db)):
    ...
    headers = [
        "Belső vonalkód", "EAN", "Termék név", "Cikkszám (SKU)", "Gyártói SKU", 
        "Egység", "ÁFA (%)", "Nettó beszerzési ár (Ft)", "Bruttó eladási ár (Ft)", "Készlet"
    ]
    ws.append(headers)
...
async def export_stocktake(stocktake_id: str, db: AsyncSession = Depends(get_db)):
    ...
    items_res = await db.execute(select(StocktakeItem).where(StocktakeItem.stocktake_id == stocktake_id))
    ...
    headers = ["Vonalkód", "Termék név", "Mennyiség", "Egységár nettó (Ft)", "Összérték nettó (Ft)"]
    ...
    return StreamingResponse(
        stream,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename=leltar_{stocktake_id}.xlsx"}
    )
