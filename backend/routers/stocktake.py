...
async def create_stocktake(
    ...
    try:
        ...
        await log_audit(
            ...
            f"Leltár létrehozva: {name} ({len(products)} tétel pillanatkép)",
        )
        await db.commit()
    ...
    await event_bus.publish(
        "inventory_events", "stocktake_created", {"id": stocktake.id, "name": name}
    )
...
async def start_stocktake(
    ...
    await db.commit()
    await log_audit(
        db, current_user.id, current_user.username, f"Leltár elindítva: {st.name}"
    )
    await event_bus.publish(
        "inventory_events",
        "stocktake_updated",
...
async def scan_item(
    ...
    if not st or st.status != StocktakeStatus.IN_PROGRESS:
        raise HTTPException(
            status_code=400, detail="A leltározás nincs aktív folyamatban állapotban!"
        )
    ...
    if not product:
        ...
        await log_audit(
            ...
            current_user.id,
            current_user.username,
            f"Ismeretlen vonalkód leltározva: {req.barcode}",
        )
...
async def list_unresolved_scans(stocktake_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        ...
        .where(
            (StocktakeUnknownBarcode.stocktake_id == stocktake_id)
            & (StocktakeUnknownBarcode.resolved.is_(False))
        )
    )
    return [
        {
...
async def link_unresolved_scan(
    unresolved_id: str,
    product_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(
    ...
    try:
        ...
        u_res = await db.execute(
            select(StocktakeUnknownBarcode)
            .where(StocktakeUnknownBarcode.id == unresolved_id)
            .with_for_update()
        )
        u_scan = u_res.scalar_one_or_none()
        if not u_scan:
            raise HTTPException(
                status_code=404, detail="Ismeretlen vonalkód rekord nem található."
            )
        if u_scan.resolved:
            raise HTTPException(
                status_code=400, detail="Ez a tétel már fel lett dolgozva."
            )
        ...
        if not product:
            raise HTTPException(
                status_code=404, detail="A megadott termék nem található."
            )

        # Find or create corresponding stocktake item
        st_res = await db.execute(
            select(StocktakeItem).where(
                (StocktakeItem.stocktake_id == u_scan.stocktake_id)
                & (StocktakeItem.product_id == product.id)
            )
        )
        ...
        await log_audit(
            ...
            current_user.id,
            current_user.username,
            f"Ismeretlen vonalkód ({u_scan.barcode}) összekapcsolva termékkel: {product.name}",
        )
        await db.commit()
    except Exception as e:
        await db.rollback()
...
async def ignore_unresolved_scan(
    ...
    result = await db.execute(
        select(StocktakeUnknownBarcode).where(
            StocktakeUnknownBarcode.id == unresolved_id
        )
    )
    ...
    await log_audit(
        ...
        current_user.id,
        current_user.username,
        f"Ismeretlen vonalkód ({u_scan.barcode}) mellőzve. Indok: {reason}",
    )
    return {"status": "success"}


@router.post("/unresolved/{unresolved_id}/reopen")
async def reopen_unresolved_scan(
    unresolved_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(
        require_role([UserRole.ADMIN, UserRole.LEADER, UserRole.WAREHOUSE])
...
async def apply_corrections(
    ...
    if not st or st.status == StocktakeStatus.APPLIED:
        raise HTTPException(
            status_code=400, detail="A leltár nem létezik vagy már javításra került!"
        )
    ...
    try:
        ...
        await log_audit(
            ...
            current_user.id,
            current_user.username,
            f"Leltár eltérések alkalmazva: {st.name} ({len(discrepancies)} termék korrigálva)",
        )
        await db.commit()
    except Exception as e:
        await db.rollback()
    ...
    await event_bus.publish(
        "inventory_events",
        "stocktake_applied",
        {"id": stocktake_id, "status": st.status.value},
    )
    return {
        "status": "success",
        "message": "A leltárkorrekciók sikeresen alkalmazva lettek az adatbázisban.",
    }
...
async def delete_stocktake(
    ...
    if st.status == StocktakeStatus.APPLIED:
        raise HTTPException(
            status_code=400,
            detail="Már véglegesített/alkalmazott leltárt nem lehet törölni!",
        )
    ...
    try:
        ...
        await log_audit(
            ...
            current_user.id,
            current_user.username,
            f"Leltár törölve: {st.name} (id: {stocktake_id})",
        )
        await db.commit()
    except Exception as e:
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Hiba történt a leltár törlése során: {str(e)}"
        )

    await event_bus.publish(
        "inventory_events", "stocktake_deleted", {"id": stocktake_id}
    )
