...
async def check_movements(req: CheckMovementsRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER, UserRole.WAREHOUSE]))):
...
async def apply_opening_stock(req: OpeningStockRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER, UserRole.WAREHOUSE]))):
    ...
    try:
        ...
        if not req.force_apply:
            ...
            if products_with_movements:
                names = [products_map[pid].name for pid in products_with_movements if pid in products_map]
                raise HTTPException(
                    status_code=409,
                    detail={
        ...
        if req.force_apply and current_user.role not in [UserRole.ADMIN, UserRole.LEADER]:
        ...
        for item in req.items:
            ...
            if not product:
                raise HTTPException(status_code=400, detail=f"Termék nem található: {item.product_id}")
            
            location = locations_map.get(item.location_id)
            if not location:
                raise HTTPException(status_code=400, detail=f"Tárhely nem található: {item.location_id}")
        ...
        await log_audit(db, current_user.id, current_user.username, f"Nyitókészlet rögzítve: {len(req.items)} db termékhez.")
        await db.commit()
    except Exception as e:
        await db.rollback()
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"Hiba a nyitókészlet rögzítése során: {str(e)}")
        
    await event_bus.publish("inventory_events", "stock_updated", {"message": "Nyitókészlet rögzítve"})
    return {"status": "success", "message": f"Nyitókészlet sikeresen rögzítve {len(req.items)} termékhez."}
...
async def download_opening_stock_template():
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=nyitokeszlet_sablon.xlsx"}
    )


@router.post("/opening-stock/import-preview")
async def import_preview(file: UploadFile = File(...), db: AsyncSession = Depends(get_db), current_user: User = Depends(require_role([UserRole.ADMIN, UserRole.LEADER, UserRole.WAREHOUSE]))):
    import io
    ...
    try:
        ...
        for row_idx, r in enumerate(rows[1:], 2):
            ...
            raw_barcode = str(r[barcode_idx]).strip() if (barcode_idx < len(r) and r[barcode_idx] is not None) else ""
            ...
            raw_sku = str(r[sku_idx]).strip() if (sku_idx < len(r) and r[sku_idx] is not None) else ""
            raw_name = str(r[name_idx]).strip() if (name_idx < len(r) and r[name_idx] is not None) else ""
            
            raw_qty_val = r[qty_idx]
            raw_loc = str(r[loc_idx]).strip() if (loc_idx < len(r) and r[loc_idx] is not None) else ""
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"Hiba a fájl feldolgozása során: {str(e)}")
