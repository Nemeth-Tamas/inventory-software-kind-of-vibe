    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.execute(
                delete(StocktakeItem).where(
                    StocktakeItem.stocktake_id.in_(
                        select(Stocktake.id).where(
                            Stocktake.name.like("TestStocktake%")
                        )
                    )
                )
            )
            await session.execute(
                delete(StocktakeUnknownBarcode).where(
                    StocktakeUnknownBarcode.stocktake_id.in_(
                        select(Stocktake.id).where(
                            Stocktake.name.like("TestStocktake%")
                        )
                    )
            ...
            await session.execute(
                delete(Stocktake).where(Stocktake.name.like("TestStocktake%"))
            )

            # 2. Delete movements by user_id
            user_ids_subq = select(User.id).where(
                User.username.in_(["test_technician", "test_admin"])
            )
            await session.execute(
                delete(InventoryMovement).where(
                    InventoryMovement.user_id.in_(user_ids_subq)
                )
            )
            ...
            await session.execute(
                delete(StocktakeUnknownBarcode).where(
                    StocktakeUnknownBarcode.barcode == "999999"
                )
            )
            await session.execute(
                delete(Product).where(
                    (Product.name == "Test Phone Screen")
                    | (Product.barcode == "260001")
                )
            )
            await session.execute(
            ...
            await session.execute(
                delete(Location).where(Location.name == "Test Location")
            )
            await session.execute(
                delete(Supplier).where(Supplier.name == "Test Supplier")
            )
            await session.execute(
                delete(AuditLog).where(
                    AuditLog.username.in_(["test_technician", "test_admin"])
                )
            )
            await session.execute(
                delete(User).where(User.username.in_(["test_technician", "test_admin"]))
            )
        ...
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url=TEST_URL
        ) as client:
            # Login as technician
            login_resp = await client.post(
                "/auth/login",
                data={"username": "test_technician", "password": "tempPass123"},
            )
            ...
            async with AsyncSessionLocal() as session:
                res_user = await session.execute(
                    select(User).where(User.username == "test_technician")
                )
            ...
            login_resp2 = await client.post(
                "/auth/login",
                data={"username": "test_technician", "password": "secureNewPass123"},
            )
            ...
            st_resp = await client.post(
                "/stocktakes?name=TestStocktake1&notes=TestNotes", headers=admin_headers
            )
            ...
            async with AsyncSessionLocal() as session:
                st_item_result = await session.execute(
                    select(StocktakeItem).where(
                        ...
                        & (StocktakeItem.product_id == product.id)
                    )
                )
            ...
            start_resp = await client.post(
                f"/stocktakes/{st_id}/start", headers=admin_headers
            )
            assert start_resp.status_code == 200

            # 6. Scan a known item
            scan_resp = await client.post(
                f"/stocktakes/{st_id}/scan",
                headers=tech_headers,
                json={"barcode": "260001", "multiplier": 2},
            )
            ...
            scan_unknown = await client.post(
                f"/stocktakes/{st_id}/scan",
                headers=tech_headers,
                json={"barcode": "999999", "multiplier": 1},
            )
            ...
            async with AsyncSessionLocal() as session:
                u_scan_res = await session.execute(
                    select(StocktakeUnknownBarcode).where(
                        StocktakeUnknownBarcode.id == unknown_scan_id
                    )
                )
            ...
            unresolved_resp = await client.get(
                f"/stocktakes/{st_id}/unresolved", headers=tech_headers
            )
            ...
            async with AsyncSessionLocal() as session:
                ...
                mov_result = await session.execute(
                    select(InventoryMovement).where(
                        ...
                        & (InventoryMovement.movement_type == MovementType.CORRECTION)
                    )
                )
            ...
            rec_resp1 = await client.post(
                "/inventory/receipt", json=receipt_payload, headers=tech_headers
            )
            assert rec_resp1.status_code == 201

            rec_resp2 = await client.post(
                "/inventory/receipt", json=receipt_payload, headers=tech_headers
            )
        ...
        async with AsyncSessionLocal() as clean_session:
            ...
            if st_id:
                await clean_session.execute(
                    delete(StocktakeUnknownBarcode).where(
                        StocktakeUnknownBarcode.stocktake_id == st_id
                    )
                )
                await clean_session.execute(
                    delete(StocktakeItem).where(StocktakeItem.stocktake_id == st_id)
                )
                await clean_session.execute(
                    delete(Stocktake).where(Stocktake.id == st_id)
                )
            if product:
                await clean_session.execute(
                    delete(Product).where(
                        (Product.id == product.id) | (Product.barcode == "260001")
                    )
                )
            ...
            if supplier:
                await clean_session.execute(
                    delete(Supplier).where(Supplier.id == supplier.id)
                )
...
async def test_master_data_operations():
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "master_data_admin", "password": "adminPass123"},
            )
            assert login_resp.status_code == 200
            ...
            cat_resp = await client.post(
                "/categories", json={"name": "Temp Cat"}, headers=headers
            )
            ...
            arch_resp = await client.post(
                f"/categories/{cat_id}/archive", headers=headers
            )
            ...
            rest_resp = await client.post(
                f"/categories/{cat_id}/restore", headers=headers
            )
            ...
            sup_resp = await client.post(
                "/suppliers", json=sup_payload, headers=headers
            )
            ...
            # Merge Categories
            cat_target_resp = await client.post(
                "/categories", json={"name": "Target Cat"}, headers=headers
            )
            ...
            merge_resp = await client.post(
                "/categories/merge",
                json={"source_id": cat_id, "target_id": cat_target_id},
                headers=headers,
            )
            ...
            del_target = await client.delete(
                f"/categories/{cat_target_id}", headers=headers
            )
        ...
        async with AsyncSessionLocal() as session:
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "master_data_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "master_data_admin")
                )
            await session.commit()
...
async def test_product_creation_success():
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "prod_success_admin", "password": "adminPass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "prod_success_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "prod_success_admin")
                )
            if category:
                await session.execute(
                    delete(Category).where(Category.id == category.id)
...
async def test_product_creation_rollback_on_failure():
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "prod_fail_admin", "password": "adminPass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
            ...
            async with AsyncSessionLocal() as session:
                res = await session.execute(
                    select(Product).where(Product.name == "Second Product")
                )
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "prod_fail_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "prod_fail_admin")
                )
            if category:
                await session.execute(
                    delete(Category).where(Category.id == category.id)
...
async def test_delete_stocktake():
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(User).where(User.username == "st_del_admin")
        )
    ...
    try:
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "st_del_admin", "password": "stDelPass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
            ...
            # Create a stocktake
            st_create_resp = await client.post(
                "/stocktakes?name=TestDeleteSt", headers=headers
            )
        ...
        async with AsyncSessionLocal() as session:
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "st_del_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "st_del_admin")
                )
            await session.commit()
...
async def test_delete_zero_stock_products():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Seed products
        prod_zero = Product(name="Test Zero Stock", barcode="999900", current_stock=0)
        prod_five = Product(name="Test Five Stock", barcode="999905", current_stock=5)
    ...
    try:
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "bulk_del_admin", "password": "bulkDelPass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
            ...
            del_resp = await client.delete(
                "/products/delete-zero-stock", headers=headers
            )
            ...
            async with AsyncSessionLocal() as session:
                res_zero = await session.execute(
                    select(Product).where(Product.barcode == "999900")
                )
                assert res_zero.scalar_one_or_none() is None

                res_five = await session.execute(
                    select(Product).where(Product.barcode == "999905")
                )
                assert res_five.scalar_one_or_none() is not None
    finally:
        async with AsyncSessionLocal() as session:
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "bulk_del_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "bulk_del_admin")
                )
            await session.execute(
                delete(Product).where(Product.barcode.in_(["999900", "999905"]))
            )
            await session.commit()
...
async def test_update_and_delete_single_product():
    ...
    async with AsyncSessionLocal() as session:
        ...
        product = Product(name="Test Single Edit", barcode="999910", current_stock=10)
    ...
    try:
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "single_prod_admin", "password": "singlePass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
        ...
        async with AsyncSessionLocal() as session:
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "single_prod_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "single_prod_admin")
                )
            await session.execute(
                delete(Product).where(Product.barcode.in_(["999910", "999990"]))
            )
            await session.commit()
...
async def test_product_archive_and_restore():
    ...
    async with AsyncSessionLocal() as session:
        ...
        product = Product(name="Test Archive Prod", barcode="999920", is_archived=False)
        session.add_all([admin_user, product])
        await session.commit()
        await session.refresh(product)
    ...
    try:
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "archive_admin", "password": "archivePass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
            ...
            arc_resp = await client.post(
                f"/products/{product.id}/archive", headers=headers
            )
            ...
            res_resp = await client.post(
                f"/products/{product.id}/restore", headers=headers
            )
        ...
        async with AsyncSessionLocal() as session:
            await session.execute(
                delete(AuditLog).where(AuditLog.username == "archive_admin")
            )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "archive_admin")
                )
...
async def test_opening_stock_workflow():
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(
            delete(InventoryMovement).where(
                InventoryMovement.product_id.in_(
        ...
        await session.execute(
            delete(AuditLog).where(AuditLog.username == "opening_admin")
        )
    ...
    async with AsyncSessionLocal() as session:
        ...
        product = Product(name="Test Opening Prod", barcode="999930", current_stock=0)
    ...
    try:
        async with httpx.AsyncClient(
            ...
            login_resp = await client.post(
                "/auth/login",
                data={"username": "opening_admin", "password": "openingPass123"},
            )
            assert login_resp.status_code == 200
            token = login_resp.json()["access_token"]
            headers = {"Authorization": f"Bearer {token}"}
            ...
            check_resp = await client.post(
                "/inventory/opening-stock/check-movements",
            ...
            apply_fail_resp = await client.post(
                "/inventory/opening-stock",
                json={
                    "items": [
                        {
            ...
            apply_force_resp = await client.post(
                "/inventory/opening-stock",
                json={
                    "items": [
                        {
        ...
        async with AsyncSessionLocal() as session:
            ...
            if product:
                ...
                await session.execute(
                    delete(Product).where(Product.barcode == "999930")
                )
            if admin_user:
                await session.execute(
                    delete(User).where(User.username == "opening_admin")
                )
            await session.execute(
                delete(Location).where(Location.name == "Opening Loc")
            )
            await session.commit()
