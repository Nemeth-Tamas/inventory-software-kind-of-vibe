...
async def test_migration_and_init_db_lifecycle(base_db_urls, monkeypatch):
    admin_engine = create_async_engine(base_db_urls["admin_async_url"], isolation_level="AUTOCOMMIT")
    ...
    try:
        ...
        new_sessionmaker = sessionmaker(new_engine, class_=AsyncSession, expire_on_commit=False)
        ...
        try:
            ...
            async with new_sessionmaker() as session:
                ...
                admin_check = await session.execute(select(User).where(User.role == UserRole.ADMIN))
            ...
            assert res_repeat.returncode == 0, f"Repeat migration failed: {res_repeat.stderr}"
            ...
            async with new_engine.connect() as connection:
                ...
                assert len(filtered_diff) == 0, f"Schema mismatch detected: {filtered_diff}"
...
async def test_migration_upgrade_path(base_db_urls, monkeypatch):
    ...
    try:
        ...
        assert res_prev.returncode == 0, f"Upgrade to previous migration failed: {res_prev.stderr}"
        ...
        assert res_head.returncode == 0, f"Upgrade from previous migration to head failed: {res_head.stderr}"
...
def test_no_adhoc_ddl_statements():
    ...
    for root, dirs, files in os.walk(backend_dir):
        ...
        for file in files:
            if not file.endswith(".py") or file.startswith("test_") or file == "test_migration_integrity.py":
                continue
